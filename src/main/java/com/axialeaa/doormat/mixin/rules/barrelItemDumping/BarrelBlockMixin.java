package com.axialeaa.doormat.mixin.rules.barrelItemDumping;
...
public abstract class BarrelBlockMixin extends BlockWithEntity implements BlockEntityProvider {
    ...
    /**
     * Ticks the barrel every tick to dump its items when the rule is enabled and the barrel is open.
     */
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return DoormatSettings.barrelItemDumping && state.get(BarrelBlock.OPEN) ?
